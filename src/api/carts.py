...
def set_item_quantity(cart_id: int, item_sku: str, cart_item: CartItem):
    with db.engine.begin() as connection:
        ...
        ), {"cart_id": cart_id, "potion_id": potion.id}).fetchone()

        if existing:
            connection.execute(sqlalchemy.text(
                "UPDATE cart_items SET quantity = :quantity WHERE cart_id = :cart_id AND potion_id = :potion_id"
            ), {"quantity": cart_item.quantity, "cart_id": cart_id, "potion_id": potion.id})
        else:
            connection.execute(sqlalchemy.text(
...
def checkout(cart_id: int, cart_checkout: CartCheckout):
    with db.engine.begin() as connection:
        ...
        connection.execute(sqlalchemy.text(
            "INSERT INTO processed_orders (order_id, endpoint, response) VALUES (:oid, 'checkout', CAST(:resp AS jsonb))"
